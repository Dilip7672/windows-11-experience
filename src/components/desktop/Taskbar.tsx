...
export function Taskbar({
  ...
  return <div className="fixed bottom-0 left-0 right-0 h-12 glass-strong flex items-center justify-center px-2 z-[1000]">
      ...
      <div className="flex items-center gap-0.5">
        ...
        {/* Search Button */}
        <button className="taskbar-icon group relative hidden sm:flex" onClick={() => {
        onSearchClick();
        ...
        return <button key={app.id} className={cn("taskbar-icon relative group", isActive && "bg-[hsl(var(--active-bg))]")} onClick={() => handleAppClick(app.id)}>
              ...
              {isOpen && <div className={cn("absolute bottom-1 left-1/2 -translate-x-1/2 h-0.5 rounded-full bg-primary transition-all", isActive ? "w-4" : "w-1.5")} />}
              <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 px-2 py-1 bg-background/90 backdrop-blur text-xs rounded opacity-0 group-hover:opacity-100 transition-opacity whitespace-nowrap pointer-events-none">
                {app.name}
              </div>
            </button>;
      ...
      <div className="flex items-center gap-1 absolute right-2">
        ...
        <button className={cn("taskbar-icon flex-col py-1 px-2 w-auto min-w-[60px] md:min-w-[80px]", isControlPanelOpen && "bg-[hsl(var(--active-bg))]")} onClick={() => {
